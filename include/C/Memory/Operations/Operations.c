...
unsigned int Hex_To_Dec(char* num){
    unsigned int ret = 0;
    for(unsigned int i = strlen(num) - 1; i >= 0; i--){
        switch (Upper(num[i])){
            case 'F':ret += 15 * (intpow(16, (strlen(num) - 1) - i));break;
            case 'E':ret += 14 * (intpow(16, (strlen(num) - 1) - i));break;
            case 'D':ret += 13 * (intpow(16, (strlen(num) - 1) - i));break;
            case 'C':ret += 12 * (intpow(16, (strlen(num) - 1) - i));break;
            case 'B':ret += 11 * (intpow(16, (strlen(num) - 1) - i));break;
            case 'A':ret += 10 * (intpow(16, (strlen(num) - 1) - i));break;
            case '9':ret += 9 * (intpow(16, (strlen(num) - 1) - i));break;
            case '8':ret += 8 * (intpow(16, (strlen(num) - 1) - i));break;
            case '7':ret += 7 * (intpow(16, (strlen(num) - 1) - i));break;
            case '6':ret += 6 * (intpow(16, (strlen(num) - 1) - i));break;
            case '5':ret += 5 * (intpow(16, (strlen(num) - 1) - i));break;
            case '4':ret += 4 * (intpow(16, (strlen(num) - 1) - i));break;
            case '3':ret += 3 * (intpow(16, (strlen(num) - 1) - i));break;
            case '2':ret += 2 * (intpow(16, (strlen(num) - 1) - i));break;
            case '1':ret += 1 * (intpow(16, (strlen(num) - 1) - i));break;
            case '0':break;
            default :continue;
        }
        if(i == 0){
...
void Dec_To_Hex(unsigned int num, char* output){
    ...
    byte digits = dc_16(num);
    int curnum;
    for (int i = 0; i < digits; i++) {
        curnum = num;
        curnum %= intpow(16, digits - i);                // Remove digits to the left
        curnum -= (curnum % intpow(16, digits - i - 1)); // Remove digits to the right
        curnum /= intpow(16, digits - i - 1);            // Isolate current digit
...
void Dec_To_Hex_Short(unsigned int num, byte* output) {
    output[0] = (num >> 24) & 0xFF;  // Most significant byte
    output[1] = (num >> 16) & 0xFF;
    output[2] = (num >> 8)  & 0xFF;
    output[3] = num & 0xFF;          // Least significant byte
}
