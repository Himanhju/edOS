...
byte ACPI_Init(IO ACPI_IO){
    RGB white = Get_RGB(0xffffff);

    byte Bus  = ACPI_IO.Device.bus;
    byte Slot = ACPI_IO.Device.slot;
    byte Func = ACPI_IO.Device.func;


    SDP = Get_SDP_addr();

    if(SDP == NULL){
        pstr_8x8("could not initialize acpi: uncampatable BIOS\n", white);
        return 1;
    }
...
byte Check_SDT(struct ACPISDTHeader *sdy){
    byte *SDT_Byte = (byte *)sdy;
    dword checksum = 0;

    for(word i = 0; i < sdy->Length; i++){
        checksum += SDT_Byte[i];
    }
