...
inline void PIT_Set_Interval(unsigned int interval){
    unsigned short divisor = 1193182 / interval;
...
inline void Set_IDT_Entry(byte entry, void *func, unsigned short selector, byte types_and_attributes){
    ...
    IDT[entry].offset_2             =   ((((unsigned int) func) >> 16) & 0xFFFF);
}
...
void IDT_Init(void){
    ...
    Set_IDT_Entry(0x26, ISR_floppy, 0x08, 0x8E);// load IRQs
